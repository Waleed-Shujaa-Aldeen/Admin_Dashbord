      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-red-200 dark:border-red-900/30">
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-y border-slate-200 dark:border-slate-800">
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{payment.payerName || payment.payerId.slice(0, 12)}</span>
          </div>
          <div className="flex justify-between items-center mb-1">
            <span className="text-sm font-semibold text-slate-600 dark:text-slate-400">Amount:</span>
            <span className="text-sm font-black text-slate-900 dark:text-slate-100">{formatCurrency(payment.amount)}</span>
          </div>
          <div className="flex justify-between items-center mb-1">
            <span className="text-sm font-semibold text-slate-600 dark:text-slate-400">Status:</span>
            <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{payment.status}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm font-semibold text-slate-600 dark:text-slate-400">Payment ID:</span>
            <span className="text-xs font-mono text-slate-500">{payment.id.slice(0, 16)}</span>
          </div>
        </div>
